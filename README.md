...
The list endpoint returns a **compact** shape per quote — `id`, `status`, `clientId`, `fullName`, `email`, `phoneNumber`, `isCustom`, `price`, `notes`, `images`, `createdAt`, `updatedAt` — and supports the standard `page` / `perPage` pagination plus:
